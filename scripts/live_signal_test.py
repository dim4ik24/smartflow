...
# smartmoneyconcepts prints ⭐ on import; on Windows cp1251 terminals this
# raises UnicodeEncodeError before main() even starts.  Reconfigure stdout/
# stderr to UTF-8 here, before any app.* import triggers that side-effect.
...
async def _diagnose_pair(symbol: str, tf: str, session) -> None:
    ...
    zone_str = "   ".join(f"{k}:{v}" for k, v in sorted(zone_counts.items())) or "—"
    ...
    print(f"  Direction (4h): {side.upper()} {arrow}")
    ...
    if derivatives:
        ...
        print("  Derivatives: none in DB  (funding / OI / LSR factors will be 0)")
    ...
    print(
        ...
        f"\n  Factors:"
        f"\n    {_ok(f.get('sweep'))}  sweep             +{settings.score_weight_sweep}"
        f"\n    {_ok(f.get('ob_retest'))}  ob_retest         +{settings.score_weight_ob_retest}"
        f"\n    {_ok(f.get('fvg'))}  fvg               +{settings.score_weight_fvg}"
        f"\n    {_ok(f.get('structure_aligned'))}  structure_aligned +{settings.score_weight_structure}"
        f"\n    {_ok(f.get('funding_extreme'))}  funding_extreme   +{settings.score_weight_funding}"
        f"  (rate={_p(f.get('funding_rate'))})"
        f"\n    {_ok(f.get('oi_rising'))}  oi_rising         +{settings.score_weight_oi_rising}"
        f"  (Δoi={_p(f.get('delta_oi'))})"
        f"\n    {_ok(f.get('lsr_confirms'))}  lsr_confirms      +{settings.score_weight_lsr}"
        f"  (l/s={_p(f.get('long_short_ratio'))})"
        f"\n    {_ok(f.get('sentiment_agrees'))}  sentiment_agrees  +{settings.score_weight_sentiment}"
        f"\n    {_ok(f.get('premium_discount'))}  premium_discount  +{settings.score_weight_premium_discount}"
    )
    print(
        f"\n  Geometry:"
    ...
    if already_analysed:
        print("\n  ✅ Score OK — engine will SKIP (already_analysed, re-run with --force)")
    else:
        print("\n  ✅ Score OK → engine will create signal + dispatch alert")


# ── Engine run ────────────────────────────────────────────────────────────────

async def _run_engine(bot) -> None:
    """Run analyze_symbol_on_close for SYMBOLS × ENTRY_TFS only."""
    ...
    for symbol in SYMBOLS:
        for tf in ENTRY_TFS:
            try:
                ...
                print(f"  ❌  engine error  {symbol}  {tf}: {exc}")
