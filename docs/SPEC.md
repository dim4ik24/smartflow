...
фінансових порад. SmartFlow — аналітичний інструмент: автоматична SMC-розмітка,
ринковий контекст (деривативи, новини, Fear & Greed), AI-пояснення сетапу.
Score = сила SMC-конфлюенсу (скільки факторів підтвердили зону) — descriptive
метрика, НЕ ймовірність виграшу. Формат сигналу:
  "ETH/USDT — лонг-сетап | Score 87/100
   Збіглось: sweep ✓ · OB-retest ✓ · structure ✓ · funding ✓ · sentiment ✓"
Заборонено формулювання виду "виросте з шансом 89%" або "WR 62%".
...
`signal_min_score = 55` — поріг офлайн-калібрування. Вимірює кількість
підтверджених SMC-факторів (сила конфлюенсу), не ймовірність виграшу.
Бектест Etap 7 не підтвердив монотонного зв'язку score → WR (non-monotonic
на 125-152 fills); поріг 55 залишається як фільтр мінімального конфлюенсу.
...
### 10.1 Функції перевірки сигналу трейдером
*(реалізація на Етапі 8, не раніше)*

**Базові (обовʼязкові для Етапу 8):**
- Мультитаймфрейм-перемикач 15m / 1h / 4h на одному символі —
  без перезавантаження сторінки, один API-запит на перемикання.
- Тултіп зони: при кліку/тачі на прямокутник або лінію зʼявляється
  попап з типом зони, ціновим діапазоном, внеском у score (наприклад
  "+20 — OB retest") і часом формування.
- Повна розбивка score за факторами на екрані деталей сигналу:
  список рядків «фактор → +N балів», сума = підсумковий score.
- Фільтри типів зон (кнопки-тоглери): OB | FVG | Ліквідність |
  Преміум/Дисконт. Стан зберігається у localStorage між сесіями.
- Маркери ключових подій на свічці: sweep (стрілка), BOS (мітка),
  CHoCH (мітка іншого кольору) — реалізовані через `setMarkers()`,
  стиль аналогічний preview_zones.py.
- Накладення EMA 50/200 (лінійні серії поверх свічок) і позначки
  об'ємних сплесків (трикутник/маркер на свічці де vol_spike=True).
  Дані вже рахуються в indicators.py і повертаються ендпоінтом /chart.

**Опційно / пізніше (після Етапу 8, якщо є попит):**
- Drag-коригування зон: трейдер може зрушити межу OB ±N% і
  зберегти корекцію локально (не впливає на серверний сигнал).
- Заморозка стану сигналу: кнопка "Snapshot" — зберігає поточний
  вигляд графіка з зонами як PNG для шерингу.
- 1-2 прості інструменти малювання: горизонтальна лінія і трендова
  лінія (тільки для власного аналізу, не зберігаються на сервері).

**СВІДОМО НЕ робимо** (див. DECISIONS.md):
- Повний редактор малювання (Фібоначчі, геометричні фігури, Pine
  Script) — це не наша цінність; для цього є TradingView.
...
- Виконано (Etap 7): walk-forward, 8 символів, 2 роки, step=4h,
  limit-order fill semantics (low ≤ mid_entry для long / high ≥ mid_entry для short).
  Результат: WR ~26-37%, PF 0.74-1.09 (нестабільний між прогонами),
  score non-monotonic → стабільний edge не підтверджено. Деталі — DECISIONS.md 2026-06-16.
- Усі скрипти і результати збережені як дослідницька база для кейс-стаді:
  backtest.py, calibrate_score.py, diagnose_sl.py, results/.
- Методологічна цінність: приклад захисту від lookahead bias (walk-forward,
  закриті свічки), zone deduplication, within-bar pessimism — документується
  у кейс-стаді незалежно від результату.
- Публічна /stats: кількість сигналів, факторний розклад, поточний ринковий
  контекст — БЕЗ обіцянок win rate.
...
- Живі метрики /stats (аптайм, к-сть проаналізованих свічок, сигналів,
  розклад SMC-факторів, ринковий контекст) — публічні, це головний доказ
  для роботодавців і клієнтів.
- "Глибока" фіча для технічних співбесід: walk-forward дослідження edge +
  захист від lookahead bias + чесний вердикт "edge не знайдено" —
  задокументувати ретельно (окремий розділ у кейс-стаді як приклад
  research integrity).
